function varargout = RedDMDTform
...
% Load the widefield image. (NIS tiff export option: RGB each Channel/grey scale/16to8bit/apply saved LUT
...
% GUI-based point selection and return reference point(fxp) and moved point(mvp).
    ...
    tform_RedDMD_Widefield = fitgeotrans(mvp, fxp, 'similarity');

    % Apply transform matrix
    Widefield_tr = imwarp(Widefield, tform_RedDMD_Widefield, 'Outputview', imref2d(size(DMDinput)));

    % 2nd registration for fine tuning.(Image contrast-based registration)
    tform_RedDMD_Widefield2 = imregtform(Widefield_tr, DMDinput, 'affine', optimizer, metric);
    Widefield_tr2 = imwarp(Widefield_tr, tform_RedDMD_Widefield2, 'Outputview', imref2d(size(DMDinput)));
...
varargout{1} = tform_RedDMD_Widefield;
varargout{2} = tform_RedDMD_Widefield2;

save tform_RedDMD_Widefield.mat tform_RedDMD_Widefield tform_RedDMD_Widefield2
close all;
end